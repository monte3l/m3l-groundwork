...
- **Least privilege.** Every GitHub Actions workflow resets permissions to
  ...
  `id-token: write`; see `CLAUDE.md`'s "Continuous integration").
...
- **Open design.** The whole toolchain, its CI, and this assurance case are
  ...
  staying secret.
- **Least common mechanism.** The graders that both feed the adoption
  report and gate this repo's own `templates/core` (`harness`/`toolchain`)
  are the _same_ rule modules, not a shared library with divergent
  copies -- a parity test enforces this so a fix applied to only one side
...
| ---------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CWE-22 (Path Traversal)                                    | `--pack <name>` and `--name <project-name>` reach filesystem `join()` calls | Both are shape-validated against an allowlist pattern in `parseArgs` before any filesystem access, rejecting path separators, `..`, and absolute paths with a usage error (exit 2). `loadPack` separately rejects any name that isn't a real `templates/packs/` directory.                                                                                 |
...
| CWE-20 (Improper Input Validation)                         | General CLI argument handling                                               | `tokenizeArgv`/`parseArgs` reject any unrecognized flag, a missing value for a value-flag, and contradictory mode flags, all before any side effect; see the CWE-22 row above for the two path-shaped values specifically.                                                                                                                                 |
